...
--unpack :: (EltRepr a ~ ((),a)) => IR a -> Operand a
--unpack (IR (OP_Pair OP_Unit (OP_Scalar x))) = x
--
--pack :: (EltRepr a ~ ((),a)) => Operand a -> IR a
--pack x = IR (OP_Pair OP_Unit (OP_Scalar x))

convertEltRepr, convertEltRepr' :: Elt t => {- dummy -} t -> x -> x
convertEltRepr _ = id
convertEltRepr' _ = id

{--
convertEltRepr :: Elt t => {- dummy -} t -> Operands (EltRepr' t) -> Operands (EltRepr t)
convertEltRepr t = cvt (eltType t) (eltType' t)
  where
    cvt :: TupleType t -> TupleType t' -> Operands t' -> Operands t
    cvt UnitTuple _ _                                                           -- (), Z, All
      = OP_Unit
    cvt (PairTuple t1 UnitTuple) (PairTuple t2 UnitTuple) (OP_Pair x OP_Unit)   -- Any (sh:.Int)
      = OP_Pair (cvt t1 t2 x) OP_Unit
    cvt (PairTuple UnitTuple (SingleTuple ta)) (SingleTuple tb) x               -- scalar types
      | Just REFL <- matchScalarType ta tb
      = OP_Pair OP_Unit x
    cvt t1 t2 x                                                                 -- tuples
      | Just REFL <- matchTupleType t1 t2
      = x
    cvt _ _ _
      = $internalError "convertEltRepr" "unhandled case"
--}
...
convertEltRepr' t = cvt (eltType t) (eltType' t)
  where
    ...
    cvt _ UnitTuple _
      = OP_Unit
    cvt (PairTuple t1 UnitTuple) (PairTuple t2 UnitTuple) (OP_Pair x OP_Unit)
      = OP_Pair (cvt t1 t2 x) OP_Unit
    cvt (PairTuple UnitTuple (SingleTuple ta)) (SingleTuple tb) (OP_Pair OP_Unit x)
      | Just REFL <- matchScalarType ta tb
      = x
    cvt t1 t2 x
      | Just REFL <- matchTupleType t1 t2
