...
defaultMap
    ...
    -> IRDelayed arch aenv (Array sh a)
    -> CodeGen   arch      (IROpenAcc arch aenv (Array sh b))
defaultMap uid aenv f a
  = transform uid aenv id f a
...
defaultBackpermute
    ...
    -> IRDelayed arch aenv (Array sh e)
    -> CodeGen   arch      (IROpenAcc arch aenv (Array sh' e))
defaultBackpermute uid aenv p a
  = transform uid aenv p id a
...
defaultTransform
    ...
    -> IRDelayed arch aenv (Array sh a)
    -> CodeGen   arch      (IROpenAcc arch aenv (Array sh' b))
defaultTransform uid aenv p f IRDelayed{..}
  = generate uid aenv . IRFun1 $ \ix -> do
      ix' <- app1 p ix
...
defaultStencil1
    ...
    -> IRDelayed  arch aenv (Array sh a)
    -> CodeGen    arch      (IROpenAcc arch aenv (Array sh b))
defaultStencil1 uid aenv f boundary arr
  = generate uid aenv . IRFun1 $ \ix -> do
...
defaultStencil2
    ...
    -> IRDelayed  arch aenv (Array sh b)
    -> CodeGen    arch      (IROpenAcc arch aenv (Array sh c))
defaultStencil2 uid aenv f boundary1 arr1 boundary2 arr2
  = generate uid aenv . IRFun1 $ \ix -> do
