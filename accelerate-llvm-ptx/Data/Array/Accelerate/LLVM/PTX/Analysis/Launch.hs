...
sharedMem p (Fold  _ x _)       blockDim = sizeOf (delayedExpType x) * (blockDim + blockDim `div` CUDA.warpSize p)
sharedMem p (Fold1 _ a)         blockDim = sizeOf (delayedAccType a) * (blockDim + blockDim `div` CUDA.warpSize p)
